...
def parse_quiz(html: str) -> List[Dict[str, Any]]:
    ...
    for match in quiz_matches:
        ...
        # Kiểm tra question không rỗng và không chỉ chứa số/ký tự đặc biệt
        if question_text and len(question_text) > 10 and all([option_a, option_b, option_c, option_d]):
            quiz_item = {
...
def parse_quiz_fallback(full_text: str) -> List[Dict[str, Any]]:
    ...
    # Loại bỏ các unwanted sections trước khi parse
    unwanted_sections = [
        r'Hiển thị đáp án.*?(?=Câu|\n\n|$)',
        r'Show answer.*?(?=Question|\n\n|$)',
        r'Xem đáp án.*?(?=Câu|\n\n|$)',
    ...
    for block in blocks:
        ...
        for line in lines:
            ...
            if any(phrase.lower() in line.lower() for phrase in ["hiển thị đáp án", "show answer", "xem đáp án"]):
                continue
            ...
            if question_match and not question and not line.startswith(('A.', 'B.', 'C.', 'D.')):
                ...
                # Chỉ chấp nhận question có độ dài hợp lý
            ...
            if option_match:
                ...
                if len(option_text) > 5:  # Đáp án phải có độ dài hợp lý
